...
function createLiFrigo (product, number){
    const li = document.createElement('li');
    const deleteButton = document.createElement('button');
    deleteButton.classList.add("delete");
    deleteButton.textContent="Borrar";
    li.textContent = `${product}:${number}:`;  
    li.appendChild(deleteButton);
    return li;
}

function createLiShop (product){
    const li = document.createElement('li');
    const deleteButton = document.createElement('button');
    deleteButton.classList.add("delete");
    deleteButton.textContent="Borrar";
...
function deleteProductFrigo(e){
    if(e.target.classList.contains("delete")){//Comprobamos que lo que hemos pulsado de la lista sea un boton con la clase añadida anteriormente
        ...
        const ArrayAux = Array.from(listaFrigo.children); //Combertimos la lista del frigorifico a array para buscar la lista correspondiente
        const index = ArrayAux.indexOf(li); //Indice del li que estamos borrando
        let cantidad = li.textContent.split(":")[1]; //Capturamos la cantidad
        const product = li.textContent.split(":")[0];//Array necesario para separar el producto del boton (nos quedamos con el producto)
        if(cantidad >1){
            cantidad = cantidad-1; 
            const liNew = createLiFrigo(product,cantidad); //Creamos un nuevo li con la cantidad restada
            listFrigo.replaceChild(liNew, listFrigo.children[index]); //Lo remplazamos por el antiguo
            frigo[product] = {product, cantidad}; //Tambien lo remplazamos en la lisa de objetos
...
/*
function deleteAddProduct(product) {
    const ArrayAux = Array.from(listaFrigo.children);
    delete list[product];
    ArrayAux.forEach(producto => {
        if(ArrayAux[producto].product === product){
            const li = ArrayAux[producto];
            listShop.removeChild(li)
        }
    })
...
formFrigo.addEventListener('submit', (e)=>{
    ...
    const cantidad = document.getElementById('cantidadFrigo').value;
    if(product){//Comprobamos que el producto no este vacio
        if(frigo.hasOwnProperty(product)){
            const ArrayAux = Array.from(listaFrigo.children);
            const lookingFor = ArrayAux.filter(producto => ArrayAux[producto].product === product)
            lookingFor[0].cantidad = lookingFor[0].cantidad + cantidad;
        }
        addListFrigo(product, cantidad); //Añadimos a la lista de firgorifico
        frigo[product] = {product, cantidad}; //Lo añadimos a la lista de objetos del frigorifico con su propio nombre como identificador
...
for(const p in frigo){
    ...
    const cantidad = frigo[p].cantidad;
    addListFrigo(product, cantidad);
}
